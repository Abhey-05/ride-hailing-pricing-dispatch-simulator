...
### 6. The AI copilot was never actually run end-to-end (Updated: now verified live)
**The problem:** `src/ai_copilot.py` and `src/copilot_tools.py` were complete and unit-tested at the tool-function layer, but the tool-calling loop itself had never been exercised with a real API key (none was available in the original build environment).
**Why an interviewer would push:** "You're claiming an AI feature works. Did you ever actually watch it answer a question?"
**Answer (now backed by a real transcript):** yes — see `docs/AI_COPILOT_TRANSCRIPT.md`, four real questions run against a live Groq-hosted model (`openai/gpt-oss-120b`), each answer checked against the documented ground truth in `docs/EXPERIMENTS.md`/`docs/ASSUMPTIONS.md` and confirmed correct, correctly sourced, and correctly distinguishing pre-computed vs. live-simulation evidence. This also validated a real architectural claim made earlier in this project's own interview prep (`docs/INTERVIEW_GUIDE.md` Q51: "the architecture is model-agnostic by design") — it was actually run on a second, different provider (Groq, not just Anthropic) using the identical system prompt and tool functions, with only the request/response translation layer differing (`src/ai_copilot.py::_ask_groq` vs. `_ask_anthropic`). One real compatibility bug was found and fixed in the process (the default Groq model wasn't available on the test account — logged in `CHANGELOG.md`).
...
4. ~~Actually exercise the AI copilot against a live API key and include a real transcript~~ — **done** (`docs/AI_COPILOT_TRANSCRIPT.md`, 4 real questions via Groq, all verified correct against documented ground truth).
